(() => {
  ...
  function destroyCharts() {
    (window.__apCharts || []).forEach((c) => {
      try { c.destroy(); } catch (_) {}
    });
    window.__apCharts = [];
  }

  window.initApDashboardCharts = function initApDashboardCharts() {
    const el = document.getElementById('ap-chart-data');
    if (!el || typeof Chart === 'undefined') return;

    destroyCharts();
    const charts = [];
    const register = (chart) => { if (chart) charts.push(chart); };
    window.__apCharts = charts;
  ...
  if (document.getElementById('ap-chart-data') && typeof Chart !== 'undefined') {
